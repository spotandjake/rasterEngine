...
export class Renderer extends Bitmap {
  ...
  public drawTriangle(v0: Vertex, v1: Vertex, v2: Vertex): void {
    ...
    if (
      ...
      !this.checkFlag(RENDER_TANGENT_SPACE)
    ) {
    ...
    if (this.checkFlag(RENDER_TANGENT_SPACE) && v0.tangent != undefined) {
      ...
      this.drawLine(
        new Vertex(pos, 0xffffff),
        new Vertex(pos.add(v0.tangent.mul(NORMAL_LENGTH)), 0xff0000)
      );
      this.drawLine(
        new Vertex(pos, 0xffffff),
        new Vertex(pos.add(v0.biTangent.mul(NORMAL_LENGTH)), 0x00ff00)
      );
      this.drawLine(
        new Vertex(pos, 0xffffff),
        new Vertex(pos.add(v0.normal.mul(NORMAL_LENGTH)), 0x0000ff)
  ...
  private modelTransform(v: Vertex): Vertex {
    ...
    const newNor =
      v.normal != undefined
        ? this.transform.mulVector(v.normal, 0.0).normalized()
        : undefined;
    const newTan =
      v.tangent != undefined
        ? this.transform.mulVector(v.tangent, 0.0).normalized()
        : undefined;
    const newBiTan =
      v.biTangent != undefined
        ? this.transform.mulVector(v.biTangent, 0.0).normalized()
        : undefined;
  ...
  private viewTransform(v: Vertex): Vertex {
    ...
    const newNor =
      v.normal != undefined
        ? this.camera.cameraTransform.mulVector(v.normal, 0.0).normalized()
        : undefined;
    const newTan =
      v.tangent != undefined
        ? this.camera.cameraTransform.mulVector(v.tangent, 0.0).normalized()
        : undefined;
    const newBiTan =
      v.biTangent != undefined
        ? this.camera.cameraTransform.mulVector(v.biTangent, 0.0).normalized()
        : undefined;
  ...
  public toggleFlag(flag: number): void {
